...
export function generateDownloadContent(payload: DownloadPayload): { content: string, contentType: string, extension: string } {
  const { title, text, themes = [], annotations = [], customFormats = [], format = 'pdf' } = payload;
...
<head>
  ...
  <style>
    ...
    @media print {
      ...
      .sticky-note-box { box-shadow: none; border-color: transparent !important; background-color: transparent !important; border-left: 4px solid; padding-left: 12px; }
      .sticky-note-box::before { display: none; }
    }
          ...
          <div class="marginalia-column">
            ${paraNotes.length > 0 ? `
              ...
              ${paraNotes.map(n => {
                ...
                return `
                <div class="sticky-note-box" style="background-color: ${noteColor}15; border-color: ${noteColor}30; border-left-color: ${noteColor};">
