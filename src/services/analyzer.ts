...
import { getDocument, getOriginal, updateDocument } from './documentStore';
...
function getClient(): GoogleGenAI {
  ...
  if (!apiKey) {
    throw new Error(
      'No GEMINI_API_KEY is configured. Add one to your .env file to use thematic analysis.'
    );
  }
...
function describeError(error: unknown): AnalysisFailure {
  ...
  if (code === 404 || has('NOT_FOUND')) {
    return {
      status: 404,
      retryable: false,
      message: `Gemini does not offer the model "${MODEL}" to this key — Google retires models fairly often. Set GEMINI_MODEL in your .env file to one your key can reach, then restart Marginalia.`
    };
  }
...
router.post('/documents/:id/analysis', async (req, res) => {
  ...
  try {
    ...
    // question the terminal answers rather than one anybody has to take on trust.
    ...
    await updateDocument(req.params.id, {
      analysis: { ...data, analysedAt: new Date().toISOString(), model: MODEL }
    });
    ...
    if (uploadedName) {
      try {
        await getClient().files.delete({ name: uploadedName });
      } catch (cleanupError) {
